...
public class TrackSelectionClass extends LinearLayout {
  ...
  private @NonNull MappedTrackInfo mappedTrackInfo;
  ...
  public TrackSelectionClass(Context context) {
    this(context, null);
  }

  /** Creates a track selection view. */
  public TrackSelectionClass(Context context, @Nullable AttributeSet attrs) {
    this(context, attrs, 0);
  }

  /** Creates a track selection view. */
  @SuppressWarnings("nullness")
  public TrackSelectionClass(
    ...
    disableView =
            (CheckedTextView)
                    inflater.inflate(R.layout.item_quality_layout, this, false);
    ...
    //addView(disableView,0);
    // Divider view.
    //addView(inflater.inflate(R.layout.divider, this, false));
    // View for clearing the override to allow the selector to use its default selection logic.
    defaultView =
            (CheckedTextView)
                    inflater.inflate(R.layout.item_quality_layout, this, false);
    defaultView.setBackgroundResource(selectableItemBackgroundResourceId);
    defaultView.setText(R.string.exo_auto);
    ...
    for (int groupIndex = 0; groupIndex < trackGroups.length; groupIndex++) {
      ...
      for (int trackIndex = 0; trackIndex < trackInfos.length; trackIndex++) {
        ...
        CheckedTextView trackView =
                (CheckedTextView) inflater.inflate(R.layout.item_quality_layout, this, false);
  ...
  private void updateViewStates() {
    ...
    if (!isDisabled && overrides.size() == 0) {
      defaultView.setBackgroundColor(getResources().getColor(R.color.blue));
      defaultView.setTextColor(getResources().getColor(R.color.white));
      defaultView.setTypeface(Typeface.create("sans-serif-regular",Typeface.NORMAL));

    } else {
      defaultView.setBackgroundColor(getResources().getColor(R.color.white));
      defaultView.setTextColor(getResources().getColor(R.color.greyTextQuality));
      defaultView.setTypeface(Typeface.create("sans-serif-light",Typeface.NORMAL));
      ...
      for (int j = 0; j < trackViews[i].length; j++) {
        if (override != null) {
          ...
          if (override.containsTrack(trackInfo.trackIndex)) {
            trackViews[i][j].setBackgroundColor(getResources().getColor(R.color.blue));
            trackViews[i][j].setTextColor(getResources().getColor(R.color.white));
            trackViews[i][j].setTypeface(Typeface.create("sans-serif-regular",Typeface.NORMAL));
          } else {
            trackViews[i][j].setBackgroundColor(getResources().getColor(R.color.white));
            trackViews[i][j].setTextColor(getResources().getColor(R.color.greyTextQuality));
            trackViews[i][j].setTypeface(Typeface.create("sans-serif-light",Typeface.NORMAL));
  ...
  private class ComponentListener implements OnClickListener {
    ...
    public void onClick(View view) {
      TrackSelectionClass.this.onClick(view);
    }
  }
